...
LastEditTime: 2024-01-15 17:20:42
...
class Main():
    ...
    def __init__(self,cfg ) -> None:
        ...
        for i in range(OD_num):
            ...
            # print('shared_distance',shared_distance)
    ...
    def main(self):
        ...
        while iter_num < 20 and error > 1e-2 : # or iter_num < 5
        # while iter_num < 1:
            iter_num += 1
            error, OD_dict = self.day_to_day(OD_dict)
            OD_dict , res  = self.run(OD_dict) 
    ...
    def day_to_day(self,  OD_dict):
        ...
        for od_id in OD_dict.keys():
            ...
            def objective_function(x, *args):
                OD_dict = args[0]  # 提取参数
                # profit
                # if np.random.rand() < fw(OD_dict, x):
                #     profit = (x * OD_dict[3] / 1000 * 2.5  - 2 * (OD_dict[5] - OD_dict[6] /2 )  /1000) * fw(OD_dict, x)
                # else:
                #     profit = 0
                profit = (x * OD_dict[3] / 1000 * 2.5  - 2 * (OD_dict[5] - OD_dict[6] /2 )  /1000) * fw(OD_dict, x)
                # print('income',(x * OD_dict[3] / 1000 * 2.5  - 2 * (OD_dict[5] - OD_dict[6] /2 )  /1000))
                # print('OD_dict[3],{},OD_dict[5],{},OD_dict[6],{},profit{},fw{},theta{}'.format(OD_dict[3],OD_dict[5],OD_dict[6],profit,fw(OD_dict, x),x))
                if profit < 0:
                    return profit
                else:
                    return -profit
            ...
            result = minimize(objective_function, x0= OD_dict[od_id][4], args=(OD_dict[od_id],), bounds=[(0.5,0.99)]) # 
            # for discount in np.linspace(0.7,0.9,10):
            #         print('fw',discount, fw(OD_dict[od_id], discount))
            # for discount in np.linspace(0.7,0.9,10):
            #     print('objective_function',discount, objective_function(discount, OD_dict[od_id]))
            ...
            OD_dict[od_id][4] = theta_dic_tmp  * alpha + (1-alpha ) * result.x[0]
            # print('theta_dic_tmp',theta_dic_tmp , 'OD_dict[od_id][4]', OD_dict[od_id][4])
    ...
    def random_main(self):
        ...
        with open('./result/day_to_day_randomness.pickle', 'wb') as file:
